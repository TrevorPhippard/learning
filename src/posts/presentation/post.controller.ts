...
export class PostController {
  ...
  @Patch(':id/reactions')
  async addReaction(@Param('id') id: string, @Body('userId') userId: string, @Body('type') type: string) {
    await this.svc.addReaction(id, userId, type);
    return { ok: true };
  }

  @Patch(':id/comments')
  async addComment(@Param('id') id: string, @Body('commentId') commentId: string, @Body('userId') userId: string, @Body('content') content: string) {
