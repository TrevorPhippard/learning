...
import { ProfileRepository, PROFILE_REPOSITORY } from '../../domain/ports/profile.repository.port';
...
export class ProfileService {
  ...
  async reorderExperience(profileId: string, experienceId: string, newIndex: number) {
    const profile = await this.repo.findById(ProfileId.of(profileId));
    if (!profile) throw new Error('Profile not found');
